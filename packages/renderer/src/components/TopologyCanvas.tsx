  ...
  return (
    <div
      ...
      style={{
        ...
        background: colors.background, cursor: dragging ? "grabbing" : "grab",
    ...
    >
      ...
      <div style={{
        ...
        background: "rgba(8,15,30,0.88)", backdropFilter: "blur(8px)",
        borderBottom: `1px solid ${colors.border}`, zIndex: 10, fontFamily: fonts.mono,
      }}>
        ...
        {(graph.meta.tags ?? []).map((tag) => (
          <span key={tag} style={{
            ...
            color: colors.green, background: colors.greenDim, borderRadius: 3, padding: "2px 8px",
          }}>{tag}</span>
        ))}
        <div style={{ flex: 1 }} />
          ...
          {legend.map((l) => (
            <div key={l.label} style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <svg width={24} height={4}>
                <line x1={0} y1={2} x2={24} y2={2} stroke={l.color} strokeWidth={1.5}
                  strokeDasharray={l.dash || "none"} />
              </svg>
              <span style={{ fontSize: 8, color: colors.textMuted, letterSpacing: "0.06em", fontWeight: 600 }}>
                {l.label}
              </span>
            </div>
          ))}
      ...
      <div style={{
        ...
        <svg width={graph.bounds.width} height={graph.bounds.height}
          style={{ position: "absolute", top: 0, left: 0, pointerEvents: "none" }}>
          {graph.groups.map((g) => <GroupOutline key={g.group.id} group={g} />)}
          {graph.edges.map((edge, i) => <ConnectionLine key={i} edge={edge} />)}
        </svg>
